...
export const reviewerService = {
  ...
  async getMyAssignments() {
    const response = await api.get('/reviewer/assignments');
  ...
  async getProposalSection(proposalId, sectionId) {
    const response = await api.get(`/reviewer/proposals/${proposalId}/sections/${sectionId}`);
  ...
  async getUploadedSection(uploadedTenderId, sectionKey) {
    const response = await api.get(`/reviewer/uploaded-tenders/${uploadedTenderId}/sections/${sectionKey}`);
    return response.data?.data;
  },
  ...
  async updateSectionContent(proposalId, sectionId, content) {
    const response = await api.put(`/reviewer/proposals/${proposalId}/sections/${sectionId}`, {
      content,
    });
    return response.data?.data;
  },
  ...
  async updateUploadedSectionContent(uploadedTenderId, sectionKey, content) {
    const response = await api.put(`/reviewer/uploaded-tenders/${uploadedTenderId}/sections/${sectionKey}`, {
      content,
    });
    return response.data?.data;
  },
