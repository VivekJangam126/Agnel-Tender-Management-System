...
export const evaluationService = {
  ...
  getTendersForEvaluation: (token) =>
    apiRequest('/api/evaluation/tenders', { token }),

  // Get bids for a specific tender
  getBidsForTender: (tenderId, token) =>
    apiRequest(`/api/evaluation/tenders/${tenderId}/bids`, { token }),

  // Initialize evaluation for a tender
  initializeTenderEvaluation: (tenderId, token) =>
    apiRequest(`/api/evaluation/tenders/${tenderId}/initialize`, { 
      method: 'POST', 
      token 
    }),
  ...
  updateBidEvaluation: (proposalId, data, token) =>
    apiRequest(`/api/evaluation/bids/${proposalId}`, {
      method: 'PUT',
      body: data,
      token
    }),
  ...
  completeEvaluation: (tenderId, token) =>
    apiRequest(`/api/evaluation/tenders/${tenderId}/complete`, {
      method: 'POST',
      token
    }),
  ...
  getTenderEvaluationDetails: (tenderId, token) =>
    apiRequest(`/api/evaluation/tenders/${tenderId}/details`, { token }),
};
